...
public class Board {
    ...
    public Player_IDs IDs; // Remove the 'final' keyword to allow reinitialization
    ...
    public void initializeGame() {
        generateBoard();
        this.IDs = new Player_IDs(); // Initialize Player_IDs
        setMatchLength(); // Set the match length before starting the game
        IDs.startRoll(turn, dice); // Determine the starting player
        movement = new Movement(dice, Pips, turn, Bar, this);
    }
    ...
    public void play() {
        ...
        while (!movement.checkWin()) {
            ...
            while (!proceed) {
                ...
                if (command.toLowerCase().equalsIgnoreCase("proceed")) {
        ...
        if (matchScore[0] < matchLength && matchScore[1] < matchLength) {
            ...
            resetBoard();
            initializeGame(); // Reset player names and match length for a new match
            play();
        } else {
