...
public class Move_Check
{
    ...
    public int canMove(int from_pip, int steps)
    {
        ...
        if(barFlag)
        {
            if((turnFlag && !Bar.getFirst().isEmpty()) || (!turnFlag && !Bar.getLast().isEmpty()))
            {
                barFlag = false;
                int result;
                if(turnFlag)
                    result = canMove(0, (((steps-1)%num_pips)+num_pips)%num_pips);
                else
                    result = canMove(23, (((steps-1)%num_pips)+num_pips)%num_pips);
                barFlag = true;
                return result;
            }
            return -2;
